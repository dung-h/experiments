...
time, plus a plan-only q40 repeat. Do not mix those seconds with CUDA-Q
`sample` wall-clock or with QPU labels.

The 25-row grid over-predicts cheap warm kernels. The 90% QFT rows from q36
onward under-predict by about 4–5×. `RUNTIME_EST` is therefore a plan-cost
proxy, not a uniformly conservative wall-clock.
